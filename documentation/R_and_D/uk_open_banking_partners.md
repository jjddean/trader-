# Top UK Open Banking Partners for TradeDNA

To execute the "Technical Service Provider" loophole described in the FCA application, TradeDNA cannot touch the payment funds. Instead, we must partner with a fully FCA-regulated **Payment Initiation Service Provider (PISP)**. 

When a user clicks "Pay HMRC Duty £4,500" in TradeDNA, our software simply tells the Partner API how much to charge and who to send it to. The Partner handles the money, assumes the regulatory liability, and deposits it with HMRC.
...
- **Why they fit TradeDNA:** TrueLayer is the European leader in Open Banking. They specialise in "Pay by Bank" (instant account-to-account transfers). Because customs duties can be extremely high (e.g., £20,000+), traditional credit card networks take massive percentage fees. TrueLayer bypasses cards entirely, allowing the SME importer to authorise a direct, instant bank payment to HMRC for pennies per transaction.
...
- **Why they fit TradeDNA:** Stripe is the easiest API to integrate. You can use Stripe to charge your users the monthly £49 SaaS fee, and *also* use their new "Financial Connections" and "Pay by Bank" APIs to route the massive HMRC duty payments. 
...
- **Why they fit TradeDNA:** Modulr isn't just a payment gateway; they are a Banking-as-a-Service (BaaS) provider. With Modulr, you could actually give every single TradeDNA user their own unique "Customs Payment Bank Account" (with a real UK sort code and account number). The importer deposits £50k into their TradeDNA wallet, and your SaaS automatically pays HMRC duties out of that wallet via Modulr's API as declarations are cleared.
- **Regulatory Status:** FCA-authorised Electronic Money Institution (EMI).

---

### Recommendation:
For Phase 5 of TradeDNA's development, **Stripe Connect** or **TrueLayer** are the absolute best starting points. 
