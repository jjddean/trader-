**Status:** ARCHIVED — history only. Do not follow as current ops.

Current: `CLAUDE.md`, `docs/hmrc/ACTIVE/tdr/environment-matrix.md`, `docs/hmrc/ACTIVE/tdr/hmrc-operations-runbook.md`, `docs/hmrc/ACTIVE/tdr/oauth-connect-troubleshooting.md`.

**Extracted facts that were still useful and verified against the repo (2026-08-29):**

- HMRC outbound calls go through `src/lib/hmrc-fetch.ts`, which uses `src/lib/rate-limiter.ts`.
- The limiter default is 3 requests/second (`HMRC_RATE_LIMIT_RPS`, else 3). Comment in that file cites HMRC API Platform max 3 rps.

**Not extracted (stale or contradicted):** Typesense as current search; Trade Test v2.0 Accept as the live default; `HMRC_ENVIRONMENT` as FreightCode application production; health body `{ status, environment, services: { convex, hmrc, clerk } }` (the route now probes Convex and returns `dependencies` / `configured`); Convex project slug `tradedna-f4430` (unverified); `HMRC_EORI` as a product default.
