...
```
docs/hmrc/ARCHIVE/trade-test/sdst-evidence-pack/forms/CDS-Production-Checklist-v1.2.odt
```
...
- **Optional:** zip `docs/hmrc/ARCHIVE/trade-test/sdst-evidence-pack/evidence/` if they ask for raw XML (usually ODT fields are enough for Trade Test)
...
| **Production Application ID** | `00292df9-e2e6-4d66-9d28-7d79a2a931ba` (from Hub URL on Get production credentials page — **not** sandbox ID) |
| **SaaS Based or On-Premise** | **SaaS** (Vercel + Convex) |
| **Push or Pull Notifications** | **Push** |
| **Production callback URL/s** | `https://www.freightcode.co.uk/api/hmrc/webhooks/notify` |
| **Sandbox callback URL** (not on ODT §1 — Hub only) | ngrok validated 2026-06-04 |
...
| LRN | Not applicable (file-upload uses MRN / Declaration ID, not LRN) |
...
- [ ] Production ID = production credentials URL UUID (not sandbox ID, not "pending")
...
- [x] Emailed to softwaredevelopersupport@service.hmrc.gov.uk — **awaiting reply**
