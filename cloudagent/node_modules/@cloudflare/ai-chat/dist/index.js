...
import { t as applyChunkToParts } from "./message-builder-uSvexDl2.js";
...
var AIChatAgent = class AIChatAgent extends Agent {
		...
		this.onMessage = async (connection, message) => {
			...
			if (typeof message === "string") {
				...
				if (data.type === MessageType.CF_AGENT_USE_CHAT_REQUEST && data.init.method === "POST") {
					...
					this._lastBody = Object.keys(customBody).length > 0 ? customBody : void 0;
					...
					return this._tryCatchChat(async () => {
						return __DO_NOT_USE_WILL_BREAK__agentContext.run({
							...
							if (response) await this._reply(data.id, response, [connection.id], { chatMessageId });
							else {
				...
				if (data.type === MessageType.CF_AGENT_STREAM_RESUME_REQUEST) {
					if (this._resumableStream.hasActiveStream()) this._notifyStreamResuming(connection);
					else connection.send(JSON.stringify({ type: MessageType.CF_AGENT_STREAM_RESUME_NONE }));
					return;
				}
				...
				if (data.type === MessageType.CF_AGENT_TOOL_RESULT) {
					...
					this._applyToolResult(toolCallId, toolName, output, overrideState, errorText).then((applied) => {
							...
							waitForStream().then(() => {
								...
								return this._tryCatchChat(async () => {
									return __DO_NOT_USE_WILL_BREAK__agentContext.run({
										...
										if (response) await this._reply(continuationId, response, [], {
											continuation: true,
								...
								console.error("[AIChatAgent] Tool continuation failed:", error);
							});
						}
					});
					return;
					...
					this._applyToolApproval(toolCallId, approved).then((applied) => {
						if (applied && autoContinue) {
							...
							waitForStream().then(() => {
								...
								console.error("[AIChatAgent] Tool approval continuation failed:", error);
							});
						}
					});
					return;
	...
	*/
	_mergeIncomingWithServerState(incomingMessages) {
		...
		const withMergedToolOutputs = serverToolOutputs.size === 0 ? incomingMessages : incomingMessages.map((msg) => {
	...
	*/
	_reconcileAssistantIdsWithServerState(incomingMessages) {
		...
		for (let i = 0; i < incomingMessages.length; i++) {
			const serverIdx = this.messages.findIndex((sm, si) => !claimedServerIndices.has(si) && sm.id === incomingMessages[i].id);
			if (serverIdx !== -1) {
		...
		return incomingMessages.map((incomingMessage, incomingIdx) => {
			...
			if (incomingMessage.role !== "assistant" || this._hasToolCallPart(incomingMessage)) return incomingMessage;
			...
			for (let i = 0; i < this.messages.length; i++) {
				...
				if (serverMessage.role !== "assistant" || this._hasToolCallPart(serverMessage)) continue;
	...
	*/
	_resolveMessageForToolMerge(message) {
		...
		for (const part of message.parts) if ("toolCallId" in part && "state" in part && (part.state === "output-available" || part.state === "output-error" || part.state === "approval-responded" || part.state === "approval-requested")) {
		...
		for (let i = 0; i < parts.length; i++) {
			...
			if (part.type === "text" && "text" in part) {
				...
				if (text.length > 1e3) {
					...
					if (AIChatAgent._byteLength(JSON.stringify(candidate)) <= AIChatAgent.ROW_MAX_BYTES) break;
				}
			}
		}
		const result = {
	...
	*/
	...
	async _streamSSEReply(id, streamId, reader, message, streamCompleted, continuation = false, abortSignal) {
		...
		if (!streamCompleted.value) {
			console.warn("[AIChatAgent] Stream was still active when cancel was received. Pass options.abortSignal to streamText() in your onChatMessage() to cancel the upstream LLM call and avoid wasted work.");
			this._completeStream(streamId);
			streamCompleted.value = true;
			this._broadcastChatMessage({
				body: "",
	...
	*/
	...
	async _reply(id, response, excludeBroadcastIds = [], options = {}) {
		...
		const abortSignal = chatMessageId ? this._chatMessageAbortControllers.get(chatMessageId)?.signal : void 0;
		return this.keepAliveWhile(() => this._tryCatchChat(async () => {
			...
			try {
				if (isSSE) await this._streamSSEReply(id, streamId, reader, message, streamCompleted, continuation, abortSignal);
				else await this._sendPlaintextReply(id, streamId, reader, message, streamCompleted, continuation, abortSignal);
			} catch (error) {
				if (!streamCompleted.value) {
			...
			if (message.parts.length > 0) if (earlyPersistedId) {
				const updatedMessages = this.messages.map((msg) => msg.id === earlyPersistedId ? message : msg);
