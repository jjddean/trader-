	...
	*/
	...
	async reconnectToStream(_options) {
		...
		return new Promise((resolve) => {
			...
			try {
				this.agent.send(JSON.stringify({ type: MessageType.CF_AGENT_STREAM_RESUME_REQUEST }));
			} catch {}
...
function useAgentChat(options) {
	const { agent, getInitialMessages, messages: optionsInitialMessages, onToolCall, onData, experimental_automaticToolResolution, tools, toolsRequiringConfirmation: manualToolsRequiringConfirmation, autoContinueAfterToolResult = true, autoSendAfterAllConfirmationsResolved = true, resume = true, body: bodyOption, prepareSendMessagesRequest, ...rest } = options;
	...
	*/
	...
	const { messages: chatMessages, setMessages, addToolResult, addToolApprovalResponse, sendMessage } = useChatHelpers;
	...
	const sendToolOutputToServer = useCallback((toolCallId, toolName, output, state, errorText) => {
		agentRef.current.send(JSON.stringify({
			...
			autoContinue: state === "output-error" ? false : autoContinueAfterToolResult,
	...
	*/
	const flushActiveStreamToMessages = useCallback((activeMsg) => {
			...
			switch (data.type) {
				...
				case MessageType.CF_AGENT_USE_CHAT_RESPONSE: {
					...
					if (!activeStreamRef.current || activeStreamRef.current.id !== data.id) {
						...
						if (isContinuation) {
							...
							for (let i = currentMessages.length - 1; i >= 0; i--) if (currentMessages[i].role === "assistant") {
								...
								if (currentMessages[i].metadata != null) existingMetadata = { ...currentMessages[i].metadata };
								break;
							}
						}
						activeStreamRef.current = {
					...
					if (data.body?.trim()) try {
						...
						if (!handled && (chunkData.type === "start" || chunkData.type === "finish" || chunkData.type === "message-metadata")) {
							if (chunkData.messageId != null && chunkData.type === "start") activeMsg.messageId = chunkData.messageId;
							if (chunkData.messageMetadata != null) activeMsg.metadata = activeMsg.metadata ? {
								...activeMsg.metadata,
					...
					if (data.done || data.error) {
						...
						activeStreamRef.current = null;
					} else if (data.replayComplete && activeMsg) flushActiveStreamToMessages(activeMsg);
					break;
				}
			}
		}
