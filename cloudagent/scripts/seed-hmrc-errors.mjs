...
const INDEX_NAME = "hmrc-cds-errors";
const EMBED_MODEL = "@cf/baai/bge-base-en-v1.5";
const DATA_PATH = path.resolve("data/hmrc-errors.json");
const BATCH_SIZE = 20;
...
async function main() {
  ...
  console.log(`Loaded ${records.length} error records`);
  ...
  for (let i = 0; i < records.length; i += BATCH_SIZE) {
    ...
    const vectors = batch.map((r, idx) => ({
      ...
      metadata: { code: r.code, message: r.message, fix: r.fix },
    }));
