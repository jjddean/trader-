    ...
    return (
      <section className="rounded-xl border border-green-200 bg-green-50 p-5">
        ...
        <p className="mt-2 text-xs font-medium text-green-900">
          {latest?.status === "completed" ? "Signed off" : "Blocked"} by{" "}
          {latest?.consultantName ?? latest?.consultantEmail}
        </p>
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="text-sm font-semibold text-black">Consultant sign-off</h2>
          <p className="mt-1 text-xs text-slate-500">
            Send a secure link to a compliance consultant. They review the packet on the web — not over email
            attachments.
          </p>
        </div>
      </div>
      ...
      {isComplete ? (
        ...
        <div className="mt-4 space-y-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div>
              <label htmlFor="consultant-name" className="text-[11px] font-medium text-slate-600">
                Name (optional)
              </label>
              <input
                id="consultant-name"
          ...
          <textarea
            ...
            rows={2}
            placeholder="Optional note for consultant…"
