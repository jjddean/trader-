...
export default function DashboardPage() {
    ...
    const verifiedLanes = lanes?.filter((l: Lane) => l.status === "Verified").length ?? 0;
    const reviewLanes = lanes?.filter((l: Lane) => l.status === "Review").length ?? 0;
    const totalSavings = lanes?.reduce((acc: number, l: Lane) => acc + (l.savingsEstimate || 0), 0) ?? 0;
        ...
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white border border-gray-200 rounded-xl p-5">
                    <p className="text-[0.625rem] font-semibold text-gray-400 uppercase tracking-widest mb-1">Trade Lanes</p>
                    <div className="flex items-baseline gap-2">
                        <h2 className="text-2xl font-light text-black">{lanes?.length ?? "—"}</h2>
                        <span className="text-[0.625rem] text-green-500 font-medium flex items-center">
                            {verifiedLanes} verified <ArrowUpRight className="h-3 w-3" />
                        </span>
                    </div>
                    <p className="text-[0.625rem] text-gray-400 mt-1">Active DCTS corridors</p>
                </div>

                <div className="bg-white border border-gray-200 rounded-xl p-5">
                    <p className="text-[0.625rem] font-semibold text-gray-400 uppercase tracking-widest mb-1">Prospects</p>
                    <div className="flex items-baseline gap-2">
                        <h2 className="text-2xl font-light text-black">{leads?.length ?? "—"}</h2>
                        <span className="text-[0.625rem] text-blue-500 font-medium">{newLeads} new</span>
                    </div>
                    <p className="text-[0.625rem] text-gray-400 mt-1">Partner pipeline</p>
                </div>
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-5">
                    ...
                    <p className="text-[0.625rem] text-gray-400 mt-1">DCTS duty relief</p>
                </div>

                <div className="bg-white border border-gray-200 rounded-xl p-5">
                    <p className="text-[0.625rem] font-semibold text-gray-400 uppercase tracking-widest mb-1">Calculations</p>
                    <h2 className="text-2xl font-light text-black">{calcHistory?.length ?? "—"}</h2>
                    <p className="text-[0.625rem] text-gray-400 mt-1">Landed cost runs</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <span className="text-[0.625rem] text-gray-400">{lanes?.length ?? 0} total</span>
                    </div>
                    {lanes && lanes.length > 0 ? (
                        <div className="divide-y divide-gray-50">
                            {lanes.slice(0, 5).map((lane: Lane) => (
                                <div key={lane._id} className="px-6 py-3 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[0.625rem] text-gray-400">{lane.commodityCode} · {lane.tier}</p>
                                        </div>
                                    </div>
                ...
                <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                        ...
                        <span className="text-[0.625rem] text-gray-400">{leads?.length ?? 0} total</span>
                    </div>
                            ...
                            {leads.slice(0, 5).map((lead: Lead) => (
                                <div key={lead._id} className="px-6 py-3 flex items-center justify-between hover:bg-gray-50/50 transition-colors">
                                    <div>
                                        ...
                                        <p className="text-[0.625rem] text-gray-400">{lead.country} · HS {lead.primaryHS}</p>
                                    </div>
                                    <span className={cn(
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-gray-50/50">
                                ...
                                <th className="px-6 py-3 text-[0.625rem] font-semibold text-gray-400 uppercase tracking-wider">Value</th>
                                <th className="px-6 py-3 text-[0.625rem] font-semibold text-gray-400 uppercase tracking-wider text-right">Landed Cost</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50">
                            {calcHistory.slice(0, 5).map((calc: Calc) => (
                                <tr key={calc._id} className="hover:bg-gray-50/50 transition-colors">
                                    <td className="px-6 py-3 text-[0.6875rem] font-mono text-gray-700">{calc.hsCode}</td>
                                    <td className="px-6 py-3 text-[0.6875rem] text-gray-600">{calc.originCountry}</td>
                                    <td className="px-6 py-3 text-[0.6875rem] text-gray-600">£{calc.value?.toLocaleString()}</td>
                                    <td className="px-6 py-3 text-[0.6875rem] font-medium text-black text-right">£{calc.totalLandedCost?.toLocaleString()}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
