    ...
    <div className="mx-auto max-w-4xl space-y-6 p-8">
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
        {notifications.length === 0 ? (
          <p className="px-6 py-12 text-center text-xs text-slate-500">No notifications stored yet.</p>
        ) : (
          <table className="w-full text-left text-sm">
            <thead className="border-b border-slate-100 bg-white">
              <tr>
                ...
                <th className="px-6 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-500">MRN</th>
                <th className="px-6 py-3 text-[10px] font-semibold uppercase tracking-wider text-slate-500">Errors</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {notifications.map((n) => (
                <tr key={n.id} className="hover:bg-slate-50/50">
                  <td className="px-6 py-3 text-[11px] font-mono text-slate-500">
                    {n.timestamp ? new Date(n.timestamp).toLocaleString("en-GB") : "—"}
                  </td>
                  <td className="px-6 py-3 text-xs font-medium text-slate-900">{n.notificationType}</td>
                  <td className="px-6 py-3 font-mono text-xs text-slate-600">{n.mrn || "—"}</td>
